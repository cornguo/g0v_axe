...
int main(void)
{
    ...
    slist = curl_slist_append(slist, "User-Agent: MSIE FireFox Chrome Safari");
    slist = curl_slist_append(slist, "Referer: http://axe-level-4.herokuapp.com/lv4/");

    for (page = 1; page <= 24; page++) {
        curl = curl_easy_init();
        if (curl) {
            sprintf(url, "http://axe-level-4.herokuapp.com/lv4/?page=%d", page);
